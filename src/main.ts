...
function renderItem(sheet: CheatSheet, section: CheatSection, item: CheatItem) {
  ...
  return `<article class="item" data-view="${escapeHtml(key)}"><div class="item-head"><strong>${escapeHtml(item.title)}</strong><span class="kind">${item.kind}</span></div>${item.description ? `<p>${escapeHtml(item.description)}</p>` : ''}${item.shortcut ? `<button class="copy" data-copy="${escapeHtml(item.shortcut)}"><kbd>${escapeHtml(item.shortcut)}</kbd></button>` : ''}${item.command ? `<button class="copy command" data-copy="${escapeHtml(item.command)}"><code>${escapeHtml(item.command)}</code></button>` : ''}${item.body ? `<p>${escapeHtml(item.body)}</p>` : ''}${item.userOwned ? `<div class="user-actions"><button data-edit="${escapeHtml(section.id)}:${escapeHtml(item.id)}">Edit</button><button class="danger" data-delete="${escapeHtml(section.id)}:${escapeHtml(item.id)}">Delete</button></div>` : ''}</article>`;
}
...
void listen<ForegroundApplication>('foreground-app', ({ payload }) => {
  ...
  query = '';
  const mapped = sheetForApplication(payload.bundleId, allSheets());
  if (mapped) selectedId = mapped;
  render();
});
