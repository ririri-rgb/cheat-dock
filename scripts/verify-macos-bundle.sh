...
if grep -q 'get-task-allow' < <(codesign -d --entitlements :- "$APP_PATH" 2>&1 || true); then
  ENTITLEMENTS="$(codesign -d --entitlements :- "$APP_PATH" 2>&1 || true)"
  if grep -A1 'com.apple.security.get-task-allow' <<<"$ENTITLEMENTS" | grep -q '<true'; then
...
printf 'Verified %s\n  identifier: %s\n  version: %s\n  minimum macOS: %s\n  architectures: %s\n  signing: %s\n' \
  "$APP_PATH" "$EXPECTED_IDENTIFIER" "$EXPECTED_VERSION" "$EXPECTED_MINIMUM" "$ARCHS" "$SIGNING_MODE"
